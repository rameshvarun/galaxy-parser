...
var identifier = tok('ID').map(token =>
...
var array_type = P.lazy(function() {
  return P.seq(base_type, P.seq(tok('['), expression, tok(']')).many()).map(parts => {
...
var binary_expression = function (operators, next) {
  ...
  var ops = operators.map(function(op) { return tok(op) });
...
var variable_declaration = P.seq(const_modifier, type_specifier, identifier, initializer.or(empty)).map(parts => {
  return ast.VariableDeclaration(range.covers(parts), parts[0] !== null, parts[1], parts[2], parts[3]);
});
...
var include = P.seq(tok('INCLUDE'), string_literal, optional_semicolon).map(function(parts) {
...
var struct_definition = P.seq(static_modifier, tok('STRUCT'), identifier, tok('{'),
  struct_field.many(), tok('}'), semicolon).map(parts =>
    ast.StructDefinition(range.covers(parts), parts[0] !== null, parts[2], parts[4]));

...
var boolean_literal = token('TRUE').or(token('FALSE')).desc("Boolean Literal");
...
var function_call = P.seq(identifier, P.token('('), actuals, P.token(')')).map(function(parts) {
  return ast.FunctionCall(range.covers(parts), parts[0], parts[2]);
});
...
var binary_expression = function (operators, next) {
  ...
  return P.seq(next, P.seq(opsParser, next).many()).map(function(parts) {
    if (parts[1].length == 0) return parts[0];
    else throw new Error();
  });
}
...
var function_declaration = P.seq(static_modifier, type_specifier, identifier, lparen,
  formals, rparen).map(function(parts) {
    return ast.FunctionDeclaration(range.covers(parts), parts[0], parts[1], parts[2], parts[4]);
  });
...
var expression_statement = expression.map(function(expr) {
  return ast.ExpressionStatement(expr.range, expr);
});
...
/** Function Definition */
var function_definition = P.seq(function_declaration, P.token('{'), statements, P.token('}')).map(function(parts) {

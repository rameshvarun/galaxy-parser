...
Parsimmon.Parser = (function() {
  ...
  function formatGot(stream, error) {
    ...
    return ' at line ' + stream[i].range.start.line + ', column '
      + stream[i].range.start.column + ', got ' + stream[i].type;
  }
  ...
  _.parse = function(stream) {
    if (!Array.isArray(stream)) {
      throw new Error('.parse must be called with a stream of tokens');
    }
    var result = this._(stream, 0);
  ...
  var token = Parsimmon.token = function(type) {
    var expected = "'" + type + "'";
    return Parser(function(stream, i) {
      var head = stream[i];
      if (head.type === type) {
